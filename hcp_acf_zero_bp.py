...
def run_script(mask_lbl):
    for task in ["Rest", "StoryM", "Motort", "Wrkmem"]:
        ...
        for scan_id, file_infos in files_dict.items():
            output_file = out_of(f"megs-hcp-alpha-{task}.acz.rois-{mask_lbl}.scan-{scan_id}.npy", False)
...
def find_files(**kwargs):
    ...
    files = glob.glob(out_of(f"megs-hcp-alpha-{task}.acz.rois-{template_lbl}.scan-*.npy", False))
    files.sort()
    return files
