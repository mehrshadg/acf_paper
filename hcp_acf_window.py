...
def do_a_file(file):
    print("Calculating ACW on %s" % file)
...
def run_script(tpt: TemplateMap):
    ...
    for task in task_order():
        ...
        for scan_id, file_infos in files_dict.items():
            output_file = out_of(f"megs-hcp-{task}.acw.rois-{tpt.name}.scan-{scan_id}.npy", False)
...
def find_files(**kwargs):
    task = kwargs["task"]
    tpt = kwargs["template"]
    files = glob.glob(out_of(f"megs-hcp-{task}.acw.rois-{tpt.name}.scan-{ANYTHING}.npy", False))
    files.sort()
    return files
