...
from neuro_helper.measurement import calc_acw
...
def do_a_file(file):
    ...
    data, freq_l, freq_h = fir_filter(data, fs, max_freq_low=8, min_freq_high=12, pass_type="bp")
    return calc_acw(data, n_job=5) / fs
...
def run_script(tpt: TemplateMap):
    ...
    for task in task_order():
        ...
        for scan_id, file_infos in files_dict.items():
            output_file = out_of(f"megs-hcp-alpha-{task}.acw.rois-{tpt.name}.scan-{scan_id}.npy", False)
...
def find_files(**kwargs):
    task = kwargs["task"]
    tpt = kwargs["template"]
    files = glob.glob(out_of(f"megs-hcp-alpha-{task}.acw.rois-{tpt.name}.scan-{ANYTHING}.npy", False))
    files.sort()
    return files
